...
const dropZoneRegex = /function DropZone\(\{[^]*?return \([\s\S]*?className=\{cn\([\s\S]*?isDragging &&[^]*?<div className="flex flex-wrap items-center justify-center gap-2 mt-2 z-10">[\s\S]*?<\/div>\s*<\/div>\n  \)\n}/m;
    ...
    <div
      ...
      className={cn(
        ...
        isDragging 
          ...
          : "border-border/60 bg-gradient-to-br from-background via-surface to-muted/20 hover:border-primary/60 hover:bg-primary/[0.03] hover:shadow-xl hover:shadow-primary/5",
    ...
    >
      ...
      <div className="relative flex items-center justify-center size-24 z-10">
        ...
        <div className={cn(
          ...
          isDragging ? "animate-ping opacity-60 scale-150" : "opacity-0 group-hover:animate-ping group-hover:opacity-40 group-hover:scale-150"
        )} />
