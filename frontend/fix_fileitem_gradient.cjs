...
const oldFileItem = /function FileItem\(\{ item, disabled, onRemove, onRetry, onCancel \}: \{ item: FileItemData; disabled\?: boolean; onRemove: \(\) => void; onRetry: \(\) => void; onCancel: \(\) => void \}\) \{[\s\S]*?return \([\s\S]*?<\/div>\n  \)\n}/m;
...
if(oldFileItem.test(content)) {
  content = content.replace(oldFileItem, newFileItem);
  fs.writeFileSync('src/components/ui/file-input.tsx', content, 'utf8');
  console.log("FileItem updated successfully");
} else {
  console.log("FileItem function not found using regex.");
}
