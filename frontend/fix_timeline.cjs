const fs = require('fs');
let content = fs.readFileSync('src/components/ui/timeline.tsx', 'utf8');

// The timeline component needs the following adjustments based on the image:
// - Inner wrapper outer ring `rounded-full border-[1px] border-border bg-transparent` inside the first div. 
// Ah, the image shows:
// The icon container is surrounded by a larger circle line that is very faint and thin. Wait, no. The image shows the icon container itself is circular and has a background and border based on its status. 
// "Registro creado" has a CheckCircle icon, it has a soft success background, and a soft success border. But wait, it looks like it has a white ring around it or it's just the bg. Let's look closer.
// It's a circle. Inside is the icon. Outer border is very thin, maybe `border-[0.5px]` or `border`. It's solid.
// But wait, the badge! "SUCCESS" badge has the new style, pill shaped. `Badge` with `appearance="soft"` but maybe `size="sm"` is fine. We should use `rounded-full` maybe if it isn't. The badge component `Badge` has `rounded-full` by default since we updated it in `badge.tsx`.
// The user says "las etiquetas usa los badges del ui kit y mejora el timeline".
// Let's modify the timeline component.
// The image shows the badges saying "SUCCESS", "PRIMARY", "WARNING", "NEUTRAL" and they are fully rounded pills.
// They also have `appearance="soft"`.
// Wait, the status `neutral` in the code says `if (item.status && item.status !== "neutral")`. But the image shows a "NEUTRAL" badge for "Caso cerrado".
// So we should show the badge even if status is "neutral".
// The line connecting them looks solid, faint grey.

const newTimeline = `import * as React from "react";
...
fs.writeFileSync('src/components/ui/timeline.tsx', newTimeline, 'utf8');
