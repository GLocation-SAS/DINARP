            ...
            {isEmpty ? (
              <div className="flex flex-col items-center justify-center py-24 px-6 text-center h-full">
                <div className="relative flex h-32 w-32 items-center justify-center mb-8">
                  ...
                  {/* Main circle */}
                  <div className="relative flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-primary via-primary/80 to-secondary shadow-[0_0_40px_rgba(var(--primary-rgb),0.3)] border border-primary-400/30 overflow-hidden">
                    ...
                    <Bell className="size-10 text-white animate-pulse" strokeWidth={2} />
                  </div>
                  
                  {/* Notification dot */}
                  <div className="absolute top-2 right-2 h-6 w-6 bg-success rounded-full border-4 border-background shadow-lg shadow-success/30 flex items-center justify-center">
                    <Check className="size-3 text-white" strokeWidth={4} />
                  </div>
                </div>
                <p className="text-h2 font-heading font-bold text-foreground mb-3 tracking-tight">Todo está al día</p>
                <p className="text-body text-muted-foreground/80 max-w-[250px]">No tienes notificaciones nuevas por el momento.</p>
              </div>
            ) : (
              <div className="flex flex-col py-2">
                {NOTIFICATIONS.map((notif) => (
                  <button 
                    ...
                    onClick={() => setOpen(false)}
                    className={cn(
                      "group relative flex w-full cursor-pointer items-start gap-4 px-5 py-4 outline-none transition-colors",
                  ...
                  >
                    ...
                    <div className={cn("flex size-12 flex-shrink-0 items-center justify-center rounded-full shadow-sm", notif.iconBg, notif.iconColor)}>
                      <notif.icon className="size-6" strokeWidth={2} />
                    </div>
                    <div className="flex flex-1 flex-col gap-1 items-start text-left">
                      <p className={cn("text-body-sm text-foreground", notif.unread ? "font-bold" : "font-semibold")}>{notif.title}</p>
                      <p className="text-body-sm text-muted-foreground line-clamp-2 pr-4 leading-relaxed">{notif.desc}</p>
                    </div>
                    <div className="flex flex-col items-end gap-2 flex-shrink-0">
                      <span className="text-caption font-medium text-muted-foreground uppercase tracking-wider">{notif.time}</span>
                      {notif.unread && (
                        <span className="size-2.5 rounded-full bg-primary ring-2 ring-primary/20 mt-1" />
                      )}
                    </div>
                  </button>
                ))}
